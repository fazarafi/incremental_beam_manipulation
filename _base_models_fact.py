...
from utils import SUMM_PAD_TOK, SUMM_START_TOK
...
class SummaryFactTrainableReranker(object):
    def __init__(self, summary_embedder, document_embedder, cfg_path, tokenizer=None):
        ...
        self.summary_embedder = summary_embedder 
        
        self.save_location = cfg.get('reranker_loc',
                                     "models/fact_reranker_{}_{}_{}".format(cfg['output_type'], self.beam_size,
    ...
    def set_up_models(self, cfg):
        ...
        docs_inputs = Input(shape=(len_docs,), name='docs_inputs')
        ...
        embed_summ = Embedding(input_dim=len_vsumm, output_dim=self.embedding_size)
        embed_docs = Embedding(input_dim=len_vdocs, output_dim=self.embedding_size)

        summ_lstm = lstm_type(self.lstm_size, return_sequences=True, return_state=True)
        docs_lstm = lstm_type(self.lstm_size, return_sequences=True, return_state=True)

        summ_lstm_out = summ_lstm(embed_summ(summ_inputs))
        docs_lstm_out = docs_lstm(embed_docs(docs_inputs))

        h_n_summ = summ_lstm_out[1:]
        h_n_docs = docs_lstm_out[1:]
        in_logistic_layer = Concatenate(axis=-1)(h_n_summ + h_n_docs + [log_probs_inputs])
        ...
        self.model = Model(inputs=[summ_inputs, docs_inputs, log_probs_inputs], outputs=output)
        self.model.compile(optimizer=optimizer, loss=loss_function)

        # self.model.summary()

    # untouched, Really? TODO FT
    ...
    def get_valid_loss(self, valid_docs_seqs, valid_summ_seqs, valid_log_probs, valid_fact_scores):
        valid_loss = 0
        for bi in range(0, valid_docs_seqs.shape[0] - self.batch_size + 1, self.batch_size):
            valid_docs_batch = valid_docs_seqs[bi:bi + self.batch_size, :]
            valid_summ_batch = valid_summ_seqs[bi:bi + self.batch_size, :]
            lp_batch = valid_log_probs[bi:bi + self.batch_size, :]
            fact_scores_batch = valid_fact_scores[bi:bi + self.batch_size, :]
            valid_loss += self.model.evaluate([valid_summ_batch, valid_docs_batch, lp_batch], fact_scores_batch,
                                              batch_size=self.batch_size, verbose=0)
        return valid_loss

    def train(self, summ_seqs, docs_seqs, fact_scores, log_probs, epoch, valid_size, min_passes=5):
        min_valid_loss = math.inf
        epoch_since_minimum = 0
        if self.output_type not in ['regression_reranker_relative', 'regression_sections']:
            summ_seqs, docs_seqs, fact_scores, log_probs = shuffle_data((summ_seqs, docs_seqs, fact_scores, log_probs))
        summ_seqs = np.array(summ_seqs)
        docs_seqs = np.array(docs_seqs)
        fact_scores = np.array(fact_scores)

        norm_log_probs = []
        for beam_start in range(0, summ_seqs.shape[0] - self.beam_size + 1, self.beam_size):
        ...
        print("Number of each input = ", summ_seqs.shape, docs_seqs.shape, log_probs.shape, fact_scores.shape)
        ...
        valid_summ_seqs = summ_seqs[-valid_size:]
        valid_docs = docs_seqs[-valid_size:]
        valid_fact_scores = fact_scores[-valid_size:]
        valid_log_probs = log_probs[-valid_size:]
        summ_seqs = summ_seqs[:-valid_size]
        docs_seqs = docs_seqs[:-valid_size]
        fact_scores = fact_scores[:-valid_size]
        log_probs = log_probs[:-valid_size]

        batch_indexes = list(range(0, summ_seqs.shape[0] - self.batch_size, self.batch_size))
        # print(batch_indexes)
        for ep in range(epoch):
            ...
            for bi in tqdm(batch_indexes):
                # print("over here 1188")
                # print("bi: ", bi)
                # print("self.batch_size: ", self.batch_size)
                
                docs_batch = docs_seqs[bi:bi + self.batch_size, :]
                summ_batch = summ_seqs[bi:bi + self.batch_size, :]
                fact_batch = fact_scores[bi:bi + self.batch_size, :]
                lp_batch = log_probs[bi:bi + self.batch_size, :]
                summ_batch, docs_batch, fact_batch, lp_batch = \
                    shuffle_data((summ_batch, docs_batch, fact_batch, lp_batch))
                summ_batch, docs_batch, fact_batch, lp_batch = \
                    np.array(summ_batch), np.array(docs_batch), np.array(fact_batch), np.array(lp_batch)
                if ep == 0 and bi == batch_indexes[0]:
                    print("Training on the following data")
                    print("Text:", summ_batch[0])
                    print("DA:", docs_batch[0])
                    print("LP:", lp_batch[0])
                    print("All Scores:", " ".join([str(x) for x in fact_batch]))
                    print("*******************************")


                # print("[DEBUG F[T] BEFORE TRAIN")
                # print("Number of each input = ", summ_batch.shape, docs_batch.shape, lp_batch.shape, fact_batch.shape)
                self.model.train_on_batch([summ_batch, docs_batch, lp_batch], fact_batch)
                losses += self.model.evaluate([summ_batch, docs_batch, lp_batch], fact_batch, batch_size=self.batch_size,
                                              verbose=0)
            # print(docs_seqs)
            # print(docs_seqs.shape)
            train_loss = losses / docs_seqs.shape[0] * self.batch_size
            time_spent = time() - start
            valid_loss = self.get_valid_loss(valid_docs, valid_summ_seqs, valid_log_probs, valid_fact_scores)
    ...
    def predict_fact_score(self, summ_seqs, docs_seqs, logprob_seqs):
        ...
        result = self.model.predict([summ_seqs, docs_seqs, logprob_seqs])
        if self.have_printed_data_count < 3:
            # print("Predicting the following")
            # print("Text:", summ_seqs[0])
            # print("DA:", docs_seqs[0])
